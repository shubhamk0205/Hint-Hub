...
CREATE POLICY "Users can view own playlist progress" ON playlist_progress
    FOR SELECT USING (auth.uid()::text = user_id);

-- Users can insert their own progress
CREATE POLICY "Users can insert own playlist progress" ON playlist_progress
    FOR INSERT WITH CHECK (auth.uid()::text = user_id);

-- Users can update their own progress
CREATE POLICY "Users can update own playlist progress" ON playlist_progress
    FOR UPDATE USING (auth.uid()::text = user_id);

-- Users can delete their own progress
CREATE POLICY "Users can delete own playlist progress" ON playlist_progress
